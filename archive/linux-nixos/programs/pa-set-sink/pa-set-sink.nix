...
{
  options = {
    programs.pa-set-sink = {
      enable = mkEnableOption "utility for globally setting pulseaudio sinks";
    };
  };

  config = mkIf config.programs.pa-set-sink.enable {
    home.packages = [
      (pkgs.writeShellScriptBin "pa-set-sink" ''
